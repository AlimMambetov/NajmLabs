...
export const Variant_4 = () => {
	...
	useEffect(() => {
		...
		const updateCanvasSize = () => {
			// Получаем размеры родительского контейнера или документа
			...
			if (parent) {
				...
				// Если нет родителя, используем размеры документа
				canvas.width = document.documentElement.scrollWidth;
				canvas.height = document.documentElement.scrollHeight;
			}

			// Переинициализируем частицы с новыми размерами
			initParticles();
		};
		...
		// Слушаем изменения размера и скролл
		updateCanvasSize();
		animate();

		window.addEventListener('resize', handleResize);
		window.addEventListener('scroll', handleResize); // Добавляем обновление при скролле
